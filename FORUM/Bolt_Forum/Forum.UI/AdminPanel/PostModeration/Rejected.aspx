<%@ Page Title="" Language="C#" MasterPageFile="~/AdminPanel/AdminMasterPage.Master"
    AutoEventWireup="true" CodeBehind="Rejected.aspx.cs" Inherits="Com.Comm100.Forum.UI.AdminPanel.PostModeration.Rejected" %>
                        ...
                        <tr class="trStyle<%#(this.rpData.Items.Count)%2%>" onmousedown="highLightRow(this)">
                            <td>
                                ...
                                <a   id="lkSubject" runat="server"  target="_blank" title='<%#GetTooltipString(Eval("Subject").ToString())%>'>
                                    <%#System.Web.HttpUtility.HtmlEncode(Com.Comm100.Framework.Common.StringHelper.GetMarkedLengthOfString(Eval("Subject").ToString(), 12))%>
                                </a></span>
                            <td>
                            ...
                            <td>
                               <a href="../Users/UserView.aspx?siteId=<%=SiteId%>&id=<%#Eval("PostUserOrOperatorId") %>" 
                                target="_blank" title="<%#GetTooltipString(Eval("PostUserOrOperatorName").ToString())%>"><span style="color: #50b846; font-weight: bold;">
                            ...
                            <td style="text-align:center">
                              <asp:ImageButton ID="btnAccept" runat="server" ImageUrl="~/Images/post approve.gif"
                                    CommandName="Accept" CommandArgument='<%#Convert.ToString(Eval("PostId"))%>' ToolTip="<%#Proxy[EnumText.enumForum_Post_HelpReApprove] %>"
                                    CausesValidation="False" OnClientClick="return reApproveConfirm();" />
