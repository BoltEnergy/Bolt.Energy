<%@ Page Title="" Language="C#" MasterPageFile="~/AdminPanel/AdminMasterPage.Master"
    AutoEventWireup="true" CodeBehind="AbuseReport.aspx.cs" Inherits="Com.Comm100.Forum.UI.AdminPanel.Abuse.AbuseReport" %>
        ...
        <div class="divTable" style="text-align: center;">
            <center>
                <table class="form-table">
                    <tr>
                        <td class="ttd">
                           <%=Proxy[EnumText.enumForum_Post_FieldSubject] %>
                        </td>
                        <td class="ctd">
                    ...
                    <tr>
                        <td class="ttd">
                            <%=Proxy[EnumText.enumForum_Abuse_FieldStatus] %>
                        </td>
                        <td class="ctd">
                ...
                <asp:Repeater ID="rptData" runat="server">
                    <ItemTemplate>
                        <tr class="trStyle<%#(this.rptData.Items.Count)%2%>" onmousedown="highLightRow(this)">
                            <td>
                                <span class="linkTopic"><a href="../../Topic.aspx?<%#GetQueryString(Convert.ToInt32(Eval("PostId")))%>&type=abuse&siteId=<%=SiteId %>&GoToPost=True#Post<%#Eval("PostId") %>" title="<%#GetTooltipString(Eval("PostSubject").ToString()) %>" target="_blank">
                                    <%#Server.HtmlEncode(Com.Comm100.Framework.Common.StringHelper.GetMarkedLengthOfString(Convert.ToString(Eval("PostSubject")), 20))%></a></span>
                            </td>
                            <td>
                            ...
                            <td>
                                <a href="../Users/UserView.aspx?siteId=<%=SiteId %>&id=<%#Eval("AbuseUserOrOperatorId")%>"
                                    target="_blank"><span style="color: #50b846; font-weight: bold;" title="<%#GetTooltipString(Eval("AbuseUserOrOperatorName").ToString())%>">
                                        <%#Server.HtmlEncode(Eval("AbuseUserOrOperatorName").ToString())%></span></a>
                            </td>
                            <td>
