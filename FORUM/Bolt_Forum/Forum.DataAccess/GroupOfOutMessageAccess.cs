...
{
    public class GroupOfOutMessageAccess
    {
        public static void AddGroupsOfOutMessage(SqlConnectionWithSiteId conn, SqlTransaction transaction, int outMessageId, List<int> userGroupIds)
        {
            ...
            strSQL = new StringBuilder("");
            strSQL.Append("insert  t_Forum_GroupOfOutMessage" + conn.SiteId + " select * from  #tmp_GroupOfOutMessage; ");
        ...
        public static void AddGroupOfOutMessage(SqlConnectionWithSiteId conn, SqlTransaction transaction, 
            int outMessageId, int userGroupId)
        {
            StringBuilder strSQL = new StringBuilder("");
            strSQL.Append(string.Format("Insert into [t_Forum_GroupOfOutMessage{0}] (OutMessageId,GroupId) values(@outMessageId,@userGroupId);", conn.SiteId));
            SqlCommand cmd = new SqlCommand(strSQL.ToString(), conn.SqlConn, transaction);
            cmd.Parameters.AddWithValue("@outMessageId", outMessageId);
