...
module.exports = function (app) {
    ...
    router.get('/project/:id', requireAuth, function (req, res) {
        var promisesArray = [];
        promisesArray.push(Producer.findOne({ projects: req.params.id }, function (err, producer) { producer }).populate({ path: 'projects', populate: { path: 'uploads' } }).populate('owner').exec(function (err, producer) { producer }));
        promisesArray.push(Project.findById(req.params.id).populate('uploads').exec());
    ...
    router.delete('/project/:projectId/images/:imageId', requireAuth, function (req, res) {
        try {
            if (objectId.isValid(req.params.projectId)) {
                var promisesArray = [];
                promisesArray.push(ProjectUpload.findById(req.params.imageId).exec());
                Promise.all(promisesArray).then(function (responseData) {
                    Project.findById(req.params.projectId).exec(function (err, proj) {
                        ProjectUpload.remove({ _id: req.params.imageId }, function (err) {
                            if (err) {
                                res.status(500).json({ message: "Error finding project upload.", error: err });
                            } else {
                                if (responseData[0].fileName != null) {
                                    for (var i = 0; i < proj.uploads.length; i++) {
                                        if (proj.uploads[i] == req.params.imageId) {
                                            proj.uploads.splice(i, 1);
                                            i = proj.uploads.length;
                                        }
                                    }
            ...
            res.status(500).json({ message: "Error removing project image.", error: e });
        }
    });
