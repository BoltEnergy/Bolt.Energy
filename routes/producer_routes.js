...
module.exports = function(app) {
  ...
  router.delete('/producers/:id/images/:imageId', requireAuth, function(req, res) {
    try {
      if (objectId.isValid(req.params.id)) {
        ProducerUpload.remove({ _id: req.params.imageId }, function(err) {
          if (err) {
            ...
            Producer.findById(req.params.id).populate('uploads').exec(function(err, user) {
              if (err) throw err;
              var index = 0;
              var found = false;
              var fileName = '';
              ...
              fs.remove(fileName);
              res.status(200).json({ message: "Producer upload removed successfully." });
            });
          }
        });
      } else {
